...
from home.views import EventViewSet, JobViewSet
...
class JobPageRenderTestCase(TestCase):
    ...
    def test_detail(self):
        response = self.client.get(
            reverse("home:job-detail", kwargs={"pk": self.job.id})
        )
...
class EventPageRenderTestCase(TestCase):
    ...
    def test_detail(self):
        response = self.client.get(
            reverse("home:event-detail", kwargs={"pk": self.event.id})
        )
        self.assertEqual(response.status_code, 200)
...
class ProfilePageRenderTestCase(TestCase):
    ...
    def test_detail(self):
        response = self.client.get(
            reverse("home:profile-detail", kwargs={"pk": self.submitter.pk})
        )
        self.assertEqual(response.status_code, 200)
