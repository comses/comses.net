...
SAMPLES_DIR = Path("library/tests/samples")
...
class ArchiveExtractorTestCase(TestCase):
    nested_code_folder = SAMPLES_DIR / "archives" / "nestedcode"
    ...
    def test_invalid_zipfile_saving(self):
        archive_name = str(SAMPLES_DIR / "archives" / "invalid.zip")
        fs_api = self.codebase_release.get_fs_api()
...
class GitRepoApiTestCase(TestCase):
    model_dir = SAMPLES_DIR / "releases" / "animals-model"
