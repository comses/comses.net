...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        qs = ComsesGroups.FULL_MEMBER.users(**criteria) if full_member else User.objects.filter(**criteria).exclude(pk=anonymous_user.pk)
        csv_writer = csv.DictWriter(sys.stdout, fieldnames=[
            'last_name', 'first_name', 'email', 'date_joined',
            'bio', 'institution', 'degrees', 'orcid_url', 'github_url',
            'research_interests', 'affiliations'
        ])
