...
TAG_CLEANUP_ACTIONS = {
    ...
    TagCleanupAction.find_by_porter_stemmer.name:
        lambda: TagCleanup.objects.bulk_create(TagCleanup.find_groups_by_porter_stemmer()),
    TagCleanupAction.find_by_platform_and_language.name:
        lambda: TagCleanup.objects.bulk_create(TagCleanup.find_groups_by_platform_and_language()),
    TagCleanupAction.delete_all_active.name:
        lambda: TagCleanup.objects.filter(transaction_id__isnull=True).delete()
}
