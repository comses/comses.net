...
class SpamClassifier:
    ...
    def preprocess_get_pad_sequences(self, column, tokenizer):
        ...
        column_padded = pad_sequences(
            column_sequences, maxlen=800, padding="post", truncating="post"  # padding
        )
    ...
    def preprocess_tokenization(self, X, isTraining=False):
        ...
        if isTraining:  # initil-train or partial-train
            if os.path.exists(tokenizer_path):  # partial-train
                ...
                for col in X.columns:
                    if col == "is_active":
                        continue
                    (
                        tokenized_X_dict[col],
                        tokenizer_dict[col],
                    ) = self.preprocess_tokenize_partial_train_column(
                        X[col], tokenizer_dict[col]
                    )
                with open(
                    "first_name_tokenizer_config2.json", "w"
                ) as fp:  # TODO erase later
                ...
                for col in X.columns:
                    ...
                    (
                        tokenized_X_dict[col],
                        tokenizer_dict[col],
                    ) = self.preprocess_tokenize_train_column(X[col])
                with open(
                    "first_name_tokenizer_config1.json", "w"
                ) as fp:  # TODO erase later
                    json.dump(tokenizer_dict["first_name"].get_config(), fp)
            ...
            for col in X.columns:
                if col == "is_active":
                    continue
                tokenized_X_dict[col], _ = self.preprocess_tokenize_test_column(
                    X[col], tokenizer_dict[col]
                )
        ...
        for col in X.columns:
            ...
            temp_df = pd.DataFrame(
                {col: tokenized_X_dict[col]}, columns=[col], index=X.index
            )
            tokenized_X[col] = temp_df

        tokenized_X["input_data"] = tokenized_X.apply(
            lambda row: self.preprocess_concatinate_tokenized_data(row), axis=1
        )
    ...
    def preprocess(self, df, mode="inference"):
        ...
        df["affiliations"] = df.apply(
            lambda row: self.preprocess_affiliations(row["affiliations"]), axis=1
        )
        df["is_active"] = df.apply(
            lambda row: self.preprocess_active_status(row["is_active"]), axis=1
        )
        ...
        if mode == "train":
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X, y, test_size=0.15, random_state=434
            )
            tokenized_X_train = self.preprocess_tokenization(
                X_train.drop(["user_id"], axis=1), isTraining
            )
            tokenized_X_test = self.preprocess_tokenization(
                X_test.drop(["user_id"], axis=1)
            )
            ...
            tokenized_X = self.preprocess_tokenization(
                X.drop(["user_id"], axis=1), isTraining
            )
    ...
    def train(self):
        ...
        # train
        prediction, metrics, model = self.train_xgboost_classifer(
    ...
    def inference(self):
        ...
        frame = {
            "user_id": uid_series.ravel(),
            "spam_likely": confidence_socres_series.ravel(),
        }
