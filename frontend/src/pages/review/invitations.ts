...
@Component({
  ...
  template: `<div>
    ...
    <p class="text-muted">
      Search by name, email address, or username among existing CoMSES Net members (<em
        >external reviewer invitation not implemented yet</em
      >)
    </p>
        ...
        <div class="col-10">
          <h2>
            ...
            <span class="pull-right">
              ...
              <button class="btn btn-danger" @click="candidate_reviewer = null" type="button">
                Cancel
              </button>
            </span>
          </h2>
    ...
    <div class="container-fluid" v-if="invitations.length > 0">
      <div class="row border-bottom py-2" v-for="invitation in invitations">
        <div class="col-xs-12 col-sm-2">
          <img
            :src="invitation.candidate_reviewer.avatar_url"
            v-if="invitation.candidate_reviewer"
        ...
        <div class="col-xs-12 col-sm-10">
          <h3>
            ...
            <span class="float-md-right">
              <button class="btn btn-outline-secondary" @click="resendEmail(invitation.slug)">
                Resend Invite
              </button>
            </span>
          </h3>
          <div class="tag-list">
            <div class="tag mx-1" v-for="tag in candidateReviewerTags(invitation)">
              {{ tag.name }}
            </div>
          </div>
        </div>
