...
export const releaseContributorSchema = yup.object().shape({
  contributor: yup
    .mixed()
    .test("is-not-null", "${path} must have a value", value => !_.isNull(value))
    .label("this"),
...
function displayContributorLabel(contributor: Contributor) {
  let name = [contributor.given_name, contributor.family_name]
    ...
    this.component.statusMessages = _.concat(
      ...
      _.zipWith(errors, this.state as CodebaseContributor[], (error, releaseContributor) => ({
        ...
        .map(value => {
          return {
            classNames: "alert alert-danger",
            message: `${displayContributorLabel(
              value.releaseContributor.contributor
...
class EditContributors extends Vue {
  ...
  public releaseContributorLabel(releaseContributor: CodebaseContributor) {
    ...
    const roles =
      releaseContributor.roles.length > 0
        ? ` (${releaseContributor.roles.map(this.roleLabel).join(", ")})`
        : "";
    return `${name}${roles}`;
  }
      ...
      this.statusMessages = [
        {
          ...
          message:
            "NOTE: there are unsaved changes to the contributor list. Click Save to apply these changes.",
        },
      ];
    } else {
      this.statusMessages = [];
