<template>
    <form v-cloak>
        <div class='row'>
            <div class="form-group col-6">
                ...
                <label style='cursor: pointer; margin-top: -20px;' for='profileUpload' class="form-control-label">
                    <img class='mt-3 d-block rounded img-fluid img-thumbnail' alt='Profile Image' v-if="state.avatar"
                         :src="state.avatar">
                    <img class='mt-3 d-block rounded img-fluid img-thumbnail' alt='Click to edit'
                         src='holder.js/150x150?text=Click to edit' v-else>
                </label>
        ...
        <c-input type="url" v-model="personal_url" name="personal_url" :errorMsgs="errors.personal_url"
                 label="Personal URL" help="A link to your personal computational modeling related website"
        ...
        <c-edit-degrees :value="degrees" @create="degrees.push($event)" @remove="degrees.splice($event, 1)"
                        @modify="degrees.splice($event.index, 1, $event.value)" name="degrees"
                        :errorMsgs="errors.degrees"
                        label="Degrees"
                        help="A list of degrees earned and their associated institutions, e.g., Ph.D., Anthropology, Arizona State University. Press enter after each degree to add them."
                        :required="config.degrees">
        </c-edit-degrees>
        <c-tagger v-model="tags" name="tags" :errorMsgs="errors.tags" label="Keywords"
                  :required="config.tags">
        </c-tagger>
        <c-checkbox v-if="!initial_full_member" :required="false" v-model="full_member" name="full_member"
                    :errorMsgs="errors.full_member"
                    label="Full Member">
            <div class="form-text text-muted" slot="help">
                By checking this box, I agree to the <a href="#" data-toggle="modal"
                                                        data-target="#rightsAndResponsibilities">rights and
                responsibilities</a> of CoMSES Net full membership.
            </div>
        </c-checkbox>
