...
from ...models import Event, Job, Model, ModelVersion, ModelKeywords, Keyword, Profile
from datetime import datetime
from django.utils import translation


def get_field(obj, field_name, attribute_name, default=''):
    if obj[field_name]:
...
class EventExtractor(Extractor):
    def _extract(self, raw_event, user_id_map: Dict[str, int]):
        return Event(
            ...
            content=get_field(raw_event, 'body', 'value'),
            early_registration_deadline=get_field(raw_event, 'field_earlyregistration', 'value', None),
            submission_deadline=get_field(raw_event, 'field_submissiondeadline', 'value', None),
...
class ProfileExtractor(Extractor):
    def _extract(self, raw_profile, user_id_map):
        return Profile(
            ...
            summary=get_field(raw_profile, 'field_profile2_research', 'value'),
            degrees='',  # TODO: change this to a text array field after moving to Postgres

            academia_edu=get_field(raw_profile, 'field_profile2_academiaedu_link', 'url'),
            blog=get_field(raw_profile, 'field_profile2_blog_link', 'url'),
            curriculum_vitae=get_field(raw_profile, 'field_profile2_cv_link', 'url'),
            institution=get_field(raw_profile, 'field_profile2_institution_link', 'url'),
            linkedin=get_field(raw_profile, 'field_profile2_linkedin_link', 'url'),
            personal=get_field(raw_profile, 'field_profile2_personal_link', 'url'),
            research_gate=get_field(raw_profile, 'field_profile2_researchgate_link', 'url'))
...
class ModelExtractor(Extractor):
    def _extract(self, raw_model, user_id_map):
